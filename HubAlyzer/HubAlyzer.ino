...
{
    Serial.print("Bluetooth event: "); Serial.println(event);
...
void setup()
{
  ...
  if (!SerialBT.begin(hostName))
    ...
    {
      Serial.print("Bluetooth initialized. Device name is "); Serial.println(hostName);
    }
  ...
  if (esp_bt_gap_set_pin(ESP_BT_PIN_TYPE_FIXED, strlen(bluetoothPinCode), reinterpret_cast<uint8_t *>(bluetoothPinCode)) == ESP_OK) {
      Serial.print("Bluetooth pin code set to "); Serial.println(bluetoothPinCode);
    }
    else {
...
#ifdef ENABLE_OTA
        EVERY_N_MILLISECONDS( 1000 ) { ArduinoOTA.handle(); }
#endif
    }
}
